...
def qIInsr(U, Psi, indices, base=2):
    ...
    for P1, P2 in bipartitions(indices):
        ...
        if rel_ent_dist < min_dist:
            ...
            # min_partition = (P1, P2)
    # return min_dist, min_partition
    return min_dist

def qIInkr(U, Psi, indices, base=2):
    """A version of quantum integrated information.
...
def qIInkc(U, indices, base=2):
    """A version of quantum integrated information.
    
    Minimizes relative entropy between Choi representations of E and E_P 
    over partitions P.
    ...
    min_dist = float('inf')
    U_choi = qt.to_choi(U)
    U_choi = U_choi / np.trace(U_choi)
    for P1, P2 in bipartitions(indices):
        U_P = partitioned_channel(U, P1, P2, indices)
        U_P_choi = qt.to_choi(U_P)
        U_P_choi = U_P_choi / np.trace(U_P_choi)
        print(U_choi)
        print(U_P_choi)
        rel_ent_dist = qt.entropy_relative(U_choi, U_P_choi, base=base)
        if rel_ent_dist < min_dist:
            min_dist = rel_ent_dist
    return min_dist

def qIInsc(U, indices, base=2):
    """A version of quantum integrated information.
    
    Minimizes QSJD between Choi representations of E and E_P 
    over partitions P.
    ...
    for P1, P2 in bipartitions(indices):
        ...
        U_P_choi = qt.to_choi(U_P)
        U_P_choi = U_P_choi / np.trace(U_P_choi)
        rel_ent_dist = sjdiv(U_choi, U_P_choi, base=base)
        if rel_ent_dist < min_dist:
            min_dist = rel_ent_dist
    return min_dist

def qIIntc(U, indices, base=2):
    """A version of quantum integrated information.
    
    Minimizes tracedist between Choi representations of E and E_P 
    over partitions P.
    ...
    for P1, P2 in bipartitions(indices):
        ...
        U_P_choi = qt.to_choi(U_P)
        U_P_choi = U_P_choi / np.trace(U_P_choi)
        rel_ent_dist = qt.tracedist(U_choi, U_P_choi)
        if rel_ent_dist < min_dist:
            min_dist = rel_ent_dist
    return min_dist

def qIInbc(U, indices, base=2):
    """A version of quantum integrated information.
    ...
    for P1, P2 in bipartitions(indices):
        ...
        print(U_choi)
        print(U_P_choi)
        assert U_choi == U_P_choi
        rel_ent_dist = qt.bures_dist(U_choi, U_P_choi)
        if rel_ent_dist < min_dist:
            min_dist = rel_ent_dist
    return min_dist
...
# def qIInsc(U, Psi, indices, base=2):
#     """A version of quantum integrated information.
